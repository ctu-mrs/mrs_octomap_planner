...
private:
  ...
  bool callbackReference([[maybe_unused]] mrs_msgs::ReferenceStampedSrv::Request &req,
...
void Pathfinder::onInit() {
  ...
  sh_position_cmd_ = mrs_lib::SubscribeHandler<mrs_msgs::PositionCommand>(shopts, "position_cmd_in", ros::Duration(3.0),
                                                                          &Pathfinder::timeoutPositionCmd, this,
                                                                          &Pathfinder::callbackPositionCmd, this);
  sh_octomap_ = mrs_lib::SubscribeHandler<octomap_msgs::Octomap>(
      shopts, "octomap_in", ros::Duration(3.0), &Pathfinder::timeoutOctomap, this, &Pathfinder::callbackOctomap, this);

  sh_mpc_prediction_ = mrs_lib::SubscribeHandler<mrs_msgs::MpcPredictionFullState>(
      shopts, "mpc_prediction_in", ros::Duration(3.0), &Pathfinder::timeoutMpcPrediction, this,
      &Pathfinder::callbackMpcPrediction, this);

  sh_control_manager_diag_ = mrs_lib::SubscribeHandler<mrs_msgs::ControlManagerDiagnostics>(
      shopts, "control_manager_diag_in", ros::Duration(3.0), &Pathfinder::timeoutControlManagerDiag, this);
...
void Pathfinder::timeoutPositionCmd(const std::string &topic, const ros::Time &last_msg,
                                    [[maybe_unused]] const int n_pubs) {
...
void Pathfinder::timeoutOctomap(const std::string &topic, const ros::Time &last_msg,
                                [[maybe_unused]] const int n_pubs) {
...
void Pathfinder::timeoutMpcPrediction(const std::string &topic, const ros::Time &last_msg,
                                      [[maybe_unused]] const int n_pubs) {
...
void Pathfinder::timeoutControlManagerDiag(const std::string &topic, const ros::Time &last_msg,
                                           [[maybe_unused]] const int n_pubs) {
...
bool Pathfinder::callbackReference([[maybe_unused]] mrs_msgs::ReferenceStampedSrv::Request &req,
                                   mrs_msgs::ReferenceStampedSrv::Response &res) {
...
void Pathfinder::timerMain([[maybe_unused]] const ros::TimerEvent &evt) {
  ...
  const bool got_position_cmd =
      sh_position_cmd_.hasMsg() && (ros::Time::now() - sh_position_cmd_.lastMsgTime()).toSec() < 2.0;
  const bool got_mpc_prediction =
      sh_mpc_prediction_.hasMsg() && (ros::Time::now() - sh_mpc_prediction_.lastMsgTime()).toSec() < 2.0;
  const bool got_control_manager_diag =
      sh_control_manager_diag_.hasMsg() && (ros::Time::now() - sh_control_manager_diag_.lastMsgTime()).toSec() < 2.0;
  const bool got_constraints =
      sh_constraints_.hasMsg() && (ros::Time::now() - sh_constraints_.lastMsgTime()).toSec() < 2.0;
  ...
  case STATE_PLANNING: {
    ...
    std::vector<double> segment_times = estimateSegmentTimes(waypoints.first, false);
    ...
    for (int i = 0; i < waypoints.first.size(); i++) {
      ...
      ref.heading = waypoints.first[i].w();
...

    {
      ...
      if (!success) {
        ...
        if (!srv_trajectory_reference.response.success) {
          ROS_ERROR("[Pathfinder]: service call for trajectory reference failed: '%s'",
                    srv_get_path.response.message.c_str());
          break;
        } else {
        }
      }
...
void Pathfinder::timerFutureCheck([[maybe_unused]] const ros::TimerEvent &evt) {
  ...
  if (control_manager_diag->flying_normally && control_manager_diag->tracker_status.have_goal) {
    ...
    for (int i = 0; i < trajectory.points.size() - 1; i++) {
      ...
      octomap::point3d point1(trajectory.points[i].position.x, trajectory.points[i].position.y,
                              trajectory.points[i].position.z);
      octomap::point3d point2(trajectory.points[i + 1].position.x, trajectory.points[i + 1].position.y,
                              trajectory.points[i + 1].position.z);
...
void Pathfinder::changeState(const State_t new_state) {
  ...
  ROS_INFO("[Pathfinder]: changing state '%s' -> '%s'", _state_names_[old_state].c_str(),
           _state_names_[new_state].c_str());
...
std::vector<double> Pathfinder::estimateSegmentTimes(const std::vector<Eigen::Vector4d> &vertices,
  ...
  const double a_max_vertical =
      std::min(constraints->vertical_ascending_acceleration, constraints->vertical_descending_acceleration);
  ...
  for (size_t i = 0; i < vertices.size() - 1; i++) {
    ...
    if (use_heading) {
      ...
      if (heading_speed_max < std::numeric_limits<float>::max() &&
          heading_acc_max < std::numeric_limits<float>::max()) {

        if (((angular_distance - (2 * (heading_speed_max * heading_speed_max) / heading_acc_max)) / heading_speed_max) <
            0) {
          hdg_velocity_time = ((angular_distance) / heading_speed_max);
        } else {
          hdg_velocity_time = ((angular_distance - (2 * (heading_speed_max * heading_speed_max) / heading_acc_max)) /
                               heading_speed_max);
        }
