...
namespace mrs_octomap_planner
{
  ...
  private:
    ...
    void callbackReference(const std::shared_ptr<mrs_msgs::srv::ReferenceStampedSrv::Request> req,
                           const std::shared_ptr<mrs_msgs::srv::ReferenceStampedSrv::Response> res);
    ...
    void callbackAddVirtualObstacle(const std::shared_ptr<mrs_msgs::srv::ValidateReferenceArray::Request> req,
                                    const std::shared_ptr<mrs_msgs::srv::ValidateReferenceArray::Response> res);
  ...
  void OctomapPlanner::initialize()
  {
    ...
    service_server_goto_ =
        node_->create_service<mrs_msgs::srv::Vec4>("~/goto_in", std::bind(&OctomapPlanner::callbackGoto, this, std::placeholders::_1, std::placeholders::_2));
  ...
  void OctomapPlanner::callbackReference(const std::shared_ptr<mrs_msgs::srv::ReferenceStampedSrv::Request> req,
                                         const std::shared_ptr<mrs_msgs::srv::ReferenceStampedSrv::Response> res)
  {
  ...
  void OctomapPlanner::callbackSetSafetyDistance(const std::shared_ptr<mrs_msgs::srv::Vec1::Request> req,
                                                 const std::shared_ptr<mrs_msgs::srv::Vec1::Response> res)
  {
    ...
    } else
    {
      ...
      RCLCPP_WARN(node_->get_logger(), "[MrsOctomapPlanner]: failed to set safety distance %.2f m (outside the allowed range [%.2f, %.2f])", req->goal,
                  _safe_obstacle_distance_min_, _safe_obstacle_distance_max_);
      res->success = false;
    }
  ...
  void OctomapPlanner::callbackAddVirtualObstacle(const std::shared_ptr<mrs_msgs::srv::ValidateReferenceArray::Request> req,
                                                  const std::shared_ptr<mrs_msgs::srv::ValidateReferenceArray::Response> res)
  {
  ...
  void OctomapPlanner::timerMain()
  {
    ...
    if (!got_octomap || !got_tracker_cmd || !got_control_manager_diag || !got_constraints)
    {
      RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "[MrsOctomapPlanner]: waiting for data: octomap = %s, position cmd = %s, ControlManager diag = %s, constraints = %s",
      ...
      } else
      {
        ...
        } else
        {
          ...
          mrs_octomap_planner::AstarPlanner planner =
              mrs_octomap_planner::AstarPlanner(node_, "MrsOctomapPlanner MRS Astar Planner", safe_obstacle_distance, _euclidean_distance_cutoff_,
      ...
      } else
      {
        ...
        { // goal unreachable
          ...
          if (dist_to_goal_prev - dist_to_goal_current < _min_dist_to_goal_improvement_)
          {
            ...
            RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                                 "[MrsOctomapPlanner]: Insufficient improvement in dist to goal. Aborting the planning to avoid oscillations.");
      ...
      for (unsigned int i = 0; i < waypoints.first.size(); i++)
      {
        ...
        if (i > 0)
        {
          ...
          if (waypoint_dist > _max_segment_length_for_heading_sampling_)
          {
            ...
            /* RCLCPP_INFO(node_->get_logger(),"[MrsOctomapPlanner]: TG inter input point %02d: [%.2f, %.2f, %.2f, %.2f]", i, inter_ref.position.x,
             * inter_ref.position.y, */
            /* inter_ref.position.z, inter_ref.heading); */
          }
        }

        RCLCPP_INFO(node_->get_logger(), "[MrsOctomapPlanner]: TG input point %02d: [%.2f, %.2f, %.2f, %.2f]", i, ref.position.x, ref.position.y,
                    ref.position.z, ref.heading);
        req_path->path.points.push_back(ref);
          ...
          for (octomap::KeyRay::iterator it1 = key_ray.begin(), end = key_ray.end(); it1 != end; ++it1)
          {
            ...
            if (node && octree->isNodeOccupied(node))
            {
              RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 100,
                                    "[MrsOctomapPlanner]: trajectory check found collision with prediction horizon between %d and %d, replanning!", i, i + 1);
              ray_is_cool = false;
              break;
            }
          }
    ...
    if (control_manager_diag->flying_normally && control_manager_diag->tracker_status.have_goal)
    {
      ...
      if (!ret)
      {
        RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                              "[MrsOctomapPlanner]: could not transform position cmd to the map frame! can not check for potential collisions!");
        return;
      }
          ...
          if (octree->computeRayKeys(point1, point_ray_end, ray))
          {
            for (octomap::KeyRay::iterator it = ray.begin(), end = ray.end(); it != end; ++it)
            {
              ...
              } else if (octree->isNodeOccupied(node))
              {
                ...
                RCLCPP_WARN(node_->get_logger(),
                            "[MrsOctomapPlanner]: Detected OCCUPIED space along the planned trajectory! Cropped the trajectory to %d from %d points.",
                ...
                } else
                {
                  ...
                  {
                    RCLCPP_ERROR(node_->get_logger(), "[MrsOctomapPlanner]: service call for trajectory reference failed: '%s'",
                                 res_traj_ref.value()->message.c_str());
                    break;
                  }
                }
      ...
      for (unsigned int i = 0; i < trajectory.points.size() - 1; i++)
      {
        ...
        if (octree->computeRayKeys(point1, point2, key_ray))
        {
          ...
          if (!ray_is_cool)
          {
            ...
            RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 100,
                                  "[MrsOctomapPlanner]: future check found collision with prediction horizon between %d and %d, hovering!", i, i + 1);
  ...
  std::optional<mrs_msgs::msg::ReferenceStamped> OctomapPlanner::getInitialCondition(const rclcpp::Time des_time)
  {
    ...
    {
      RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                            "[MrsOctomapPlanner]: could not obtain initial condition, the input_id (%lu) does not match id of last sent path (%d).",
    ...
    {
      RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                            "[MrsOctomapPlanner]: could not obtain initial condition, the desired time is too far in the future");
      return {};
    }
    ...
    for (const auto& obst : virtual_obstacles_)
    {
      ...
      if (obst.frame_id != octree_frame_)
      {
        RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                             "[MrsOctomapPlanner] not adding virtual obstacle because it's frame_id (%s) doesn't match octree_frame (%s)",
